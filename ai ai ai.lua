...
cmd_table[#cmd_table+1] = {
    ...
    ctype = {CMD_TYPE.NONE},
    load = function()
        if Services.Players.LocalPlayer.Character and Services.Players.LocalPlayer.Character.Humanoid.Health > 0 then
...
cmd_table[#cmd_table+1] = {
    ...
    load = function()
        ...
        local c = CoroutineManager:Create("chams1", function()
            for _,v in ipairs (Services.Players:GetPlayers()) do
                for j,k in ipairs (v.Character:GetChildren()) do
                    if k:IsA("BasePart") or k:IsA("MeshPart") and k.Transparency > 0 then
                        CreateBox(k)
                    end
                end
            end
...
cmd_table[#cmd_table+1] = {
    ...
    load = function()
        local c = CoroutineManager:Create("chams1", function()
            for _,v in ipairs (Services.Players:GetPlayers()) do
                for j,k in ipairs (v.Character:GetChildren()) do
                    if k:IsA("BasePart") or k:IsA("MeshPart") and k.Transparency > 0 and k:FindFirstChild("BoxHandleAdornment") then
                        k:FindFirstChild("BoxHandleAdornment"):Destroy()
                    end
                end
            end
        end)
...
cmd_table[#cmd_table+1] = {
    names = {"fog","setfog","changefog"},
    desc = "Changes the fog",
    ctype = {CMD_TYPE.NUMBER},
    load = function(args)
...
cmd_table[#cmd_table+1] = {
    names = {"maxslopeangle"},
    desc = "Changes the slope angle",
    ctype = {CMD_TYPE.NUMBER},
    load = function(args)
        if Services.Players.LocalPlayer.Character and Services.Players.LocalPlayer.Character.Humanoid then
            Services.Players.LocalPlayer.Character.Humanoid.MaxSlopeAngle = args[1]
        end
    end,
}
